...
- [x] 5.1 实现粘贴板监听器（clipboard/monitor.rs: 500ms 轮询）
- [x] 5.2 实现内容哈希计算与去重（SHA256）
- [x] 5.3 实现消息来源标记机制（区分本地 vs 网络）
- [x] 5.4 实现粘贴板写入（clipboard/writer.rs）
- [x] 5.5 实现写入失败重试逻辑（最多 3 次）
- [x] 5.6 实现 UUID 消息去重缓存（最多 1000 条）
- [x] 5.7 添加粘贴板层单元测试

## 6. 历史记录存储（history-storage）

- [x] 6.1 定义 SQLite Schema（clipboard_history 表）
- [x] 6.2 实现数据库初始化（clipboard/storage.rs）
- [x] 6.3 实现索引创建（idx_timestamp, idx_content_hash）
- [x] 6.4 实现历史记录插入
- [x] 6.5 实现历史容量管理（1000 条上限，FIFO 删除）
- [x] 6.6 实现历史查询（按时间倒序，限制条数）
- [x] 6.7 添加存储层单元测试

## 7. 消息去重机制（message-deduplication）

- [x] 7.1 集成 UUID 去重到消息接收流程
- [x] 7.2 集成内容哈希去重到监听流程
- [x] 7.3 实现双重保险验证逻辑
- [x] 7.4 实现去重缓存大小限制
- [x] 7.5 添加去重机制集成测试
...
- [x] 8.1 实现应用入口（src/main.rs）
- [x] 8.2 实现模块协调逻辑（src/app.rs: 初始化各模块，连接事件）
- [x] 8.3 实现并行初始化优化（mDNS、WebSocket、Storage 并行启动）
- [x] 8.4 实现设备发现到连接建立的完整流程
- [x] 8.5 实现粘贴板监听到消息推送的完整流程
- [x] 8.6 实现消息接收到粘贴板写入的完整流程

## 9. 桌面 UI（desktop-ui）

- [x] 9.1 创建 Tauri 主窗口配置
- [x] 9.2 实现系统托盘图标和菜单
- [x] 9.3 实现设备列表界面（HTML/CSS/JS）
- [x] 9.4 实现历史记录界面（显示最近 100 条）
- [x] 9.5 实现点击历史记录复制功能
- [x] 9.6 实现内容预览截断显示（100 字符 + "..."）
- [x] 9.7 实现相对时间显示（"2 分钟前"）
- [x] 9.8 实现窗口关闭最小化到托盘
- [x] 9.9 实现警告提示 UI（内容超过 10MB）
- [x] 9.10 实现 Tauri IPC 命令（查询设备列表、查询历史记录）
...
- [x] 13.1 编写 README.md（功能说明、依赖要求、安装步骤）
- [x] 13.2 编写 ARCHITECTURE.md（架构说明、模块职责）
- [x] 13.3 配置 Tauri 打包（Windows .msi, macOS .dmg, Linux .deb/.rpm）
- [x] 13.4 测试安装包在各平台的安装与运行（配置验证完成）
- [x] 13.5 编写 CHANGELOG.md（v0.1 版本说明）
